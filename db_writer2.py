...
def sasl_conf():
    return {
        ...
        'group.id': 'group1',  # ✅ Same group as db_writer1
...
def main(topic):
    ...
    counter = 0
    print(f"Consumer 2 started. Listening to: {topic}")
    ...
    while True:
        try:
            ...
            try:
                ...
                counter += 1
                print(f"\n{'='*50}")
                print(f"Consumer 2 - Record {counter}")
                print(f"Data: {bid}")
                ...
                cnx.commit()
                print(f"Consumer 2 - Inserted successfully!")
                cur.close()
                cnx.close()
                print(f"{'='*50}")
